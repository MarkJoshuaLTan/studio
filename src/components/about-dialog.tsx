...
export function AboutDialog() {
  ...
  return (
    <Dialog>
      ...
      <DialogContent 
        showClose={false}
        className="max-w-2xl w-[95vw] h-[85vh] flex flex-col p-0 overflow-hidden border-0 glass-container bg-black/90 backdrop-blur-3xl shadow-2xl"
      >
        ...
        <DialogHeader className="p-8 pb-4">
          <DialogTitle className="flex items-center gap-3 text-3xl font-extrabold tracking-tight">
            <Building2 className="h-8 w-8 text-primary drop-shadow-[0_0_8px_rgba(34,197,94,0.5)]" />
            About TaxWise Parañaque
          </DialogTitle>
          <DialogDescription className="text-neutral-400 text-lg font-medium">
        ...
        <div className="flex-1 overflow-y-auto p-8 pt-2 scrollbar-thin scrollbar-thumb-primary/20">
          <div className="space-y-10 pb-10">
              ...
              <div className="bg-white/[0.03] p-6 rounded-2xl border border-white/10 space-y-4">
                ...
                <ul className="space-y-4">
                  <li className="flex gap-3 text-sm text-neutral-300">
                    ...
                    <span><strong className="text-white">Uniform Standards:</strong> Adopting international valuation standards to ensure fairness and consistency across all barangays.</span>
                  </li>
                  <li className="flex gap-3 text-sm text-neutral-300">
                    <ArrowRight className="h-4 w-4 mt-0.5 shrink-0 text-primary" />
                    <span><strong className="text-white">Economic Growth:</strong> Enhancing investor confidence by providing a predictable and stable tax environment.</span>
                  </li>
                  <li className="flex gap-3 text-sm text-neutral-300">
                    <ArrowRight className="h-4 w-4 mt-0.5 shrink-0 text-primary" />
                    <span><strong className="text-white">Transparency:</strong> Making the market value of properties public and easily verifiable by all stakeholders.</span>
                  </li>
                </ul>
              </div>
            </section>
            ...
            <section className="space-y-4">
              <h3 className="text-lg font-bold text-white">Team Contacts</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {developers.map((dev) => (
                  <a 
                    ...
                    className="flex items-center gap-3 text-sm font-semibold p-3 rounded-xl bg-white/[0.03] border border-white/5 hover:border-primary/30 hover:bg-white/[0.06] transition-all group"
                  >
                    <div className="h-2 w-2 rounded-full bg-primary group-hover:scale-125 transition-transform" />
                    <span className="text-neutral-300 group-hover:text-white">{dev.name}</span>
                  </a>
                ))}
              </div>
            </section>
        ...
        <div className="p-8 border-t border-white/10 bg-black/40 flex flex-col items-center gap-1">
